...
# AOI boundary shapefile (INTERNAL - NOT UPLOADED BY USER)
INTERNAL_BOUNDARY_PATH = "Tirupati_Boundary/Tirupati_fixed.shp"  # Fixed boundary that overlaps with rasters
...
def run_automated_pipeline(safe_t1_folder: str, safe_t2_folder: str):
    ...
    print(f"Boundary: {INTERNAL_BOUNDARY_PATH} (INTERNAL)")
    print("\n" + "="*70)
    ...
    try:
        ...
        boundary = gpd.read_file(INTERNAL_BOUNDARY_PATH)
        if boundary.crs != image_crs:
            boundary = boundary.to_crs(image_crs)
        ...
        X_T1, metadata_T1 = preprocessor_t1.preprocess(tif_t1, INTERNAL_BOUNDARY_PATH)
        print(f"🔍 DEBUG: X_T1 shape = {X_T1.shape} (should be (pixels, 5_features))")
        
        preprocessor_t2 = ImagePreprocessor("T2", shared_stats)
        X_T2, metadata_T2 = preprocessor_t2.preprocess(tif_t2, INTERNAL_BOUNDARY_PATH)
